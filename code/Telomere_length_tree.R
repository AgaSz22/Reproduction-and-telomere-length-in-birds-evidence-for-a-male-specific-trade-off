...
### Date: 29/06/2026
### Aim: Build and visualise a phylogenetic tree for the telomere length dataset
...
names_species    <- tnrs_match_names(data_tl$Species)
...
m.brm_zr_values <- readRDS("../rds_objects/m.brm_zr_values.rds")

# ── Extract fixed effects & CIs ────────────────────────────────────────────

fixed_summary <- as.data.frame(summary(m.brm_zr_values)$fixed)
n_species     <- nrow(fixed_summary)         

telomeres     <- fixef(m.brm_zr_values)
...
common_names <- unique(dplyr::select(data_tl, Species, Common_name))
...
ggsave(
  filename = "../output_figures/telomere_length_phylo_tree.svg",
  plot     = tree,
  device   = "svg",
  width    = 190,       
  height   = 148,        
